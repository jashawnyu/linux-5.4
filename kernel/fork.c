...
/* #endif */
	...
	//do nothing
	kcov_task_init(
		tsk); //代码覆盖率（Code Coverage）是反映测试用例对被测软件覆盖程序的重要指标，kcov是用于随机测试中合适表达代码覆盖率的一种工具
	     ...
	     struct kernel_clone_args *args)
{
	...
	if (clone_flags & CLONE_THREAD) {
		if ((clone_flags & (CLONE_NEWUSER | CLONE_NEWPID)) ||
		    (task_active_pid_ns(current) != //进程所属的进程号命名空间
...
long _do_fork(struct kernel_clone_args *args)
{
	...
	if (!(clone_flags & CLONE_UNTRACED)) {
		...
		else if (args->exit_signal !=
			 SIGCHLD) /*fork系统调用中exit_signal被赋值为SIGCHLD*/
