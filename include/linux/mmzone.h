...
#ifndef CONFIG_FORCE_MAX_ZONEORDER //1
#define MAX_ORDER 11
#else
 ...
 */
#define PA_SECTION_SHIFT	(SECTION_SIZE_BITS)  // 30
#define PFN_SECTION_SHIFT	(SECTION_SIZE_BITS - PAGE_SHIFT) //18

#define NR_MEM_SECTIONS		(1UL << SECTIONS_SHIFT)//(1<<18)

#define PAGES_PER_SECTION       (1UL << PFN_SECTION_SHIFT)
#define PAGE_SECTION_MASK	(~(PAGES_PER_SECTION-1))  //1100 0000 0000 0000 0000
...
/*SECTIONS_PER_ROOT即一个物理页面可以存放多少"struct mem_section"实例，由于目前内存是按物理页面来管理的，所以一次会分配一个物理页面来存放"struct mem_section"实例，称为一个ROOT，如果"struct mem_section"实例很多的话，可能需要分配多个物理页面 */
...
#define NR_SECTION_ROOTS	DIV_ROUND_UP(NR_MEM_SECTIONS, SECTIONS_PER_ROOT) //(0x40000,0x1000)   (((0x40000) + (256) - 1) / (256)) = 512
